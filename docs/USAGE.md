...
- **Value List**（值列表）。
- **懸空的幾何參數**：畫布上獨立放置、有接到下游元件、但自己沒有上游來源（Source）的一般參數物件（例如一個接了線但沒有連任何東西進來的 Brep/Point/Geometry 參數）。這類物件無法從檔案內容直接判斷型別名稱，候選列的「型別」會顯示該物件自己的元件名稱。

**候選輸出：**
- 沒有任何下游接線的 Panel 或一般參數物件——「來自」欄會顯示是哪個元件的哪個輸出腳位餵給它的（`fed_by`）。
...
### 2.6 文字輸出：一律經幾何 + AttributeUserText
...
### 2.7 單位陷阱：compute.geometry 預設 Millimeters
...
| 症狀 | 可能原因 / 排查方式 |
|---|---|
| 輸出全部是空值，但沒有任何錯誤訊息 | 最常見的原因（依可能性排序）：① 輸入格式不對（幾何要用 `file_3dm`/`encoded`，且需先確認能載入非空物件）；② **模型單位不一致**——GH 檔案是 Meters、Rhino.Compute 卻用 Millimeters 解讀，幾何被縮放 1000 倍後分析元件靜默回空值（見第 2.7 節，可從測試頁籤的 `modelunits` 提示確認）；③ 自動轉換勾選的候選名稱跟呼叫時給的參數名不一致（或手動標記時 Hops Get 元件的 NickName 對不上）；④ 手動標記時用了**非標準的 Hops Get 元件**（例如外掛自帶的替代品，見第 2.5 節）——這種情況 debug 模式的 `raw` 回應通常也看不出異常，因為 Rhino.Compute 本身沒有報錯，只是該輸入完全沒有被填入資料 |
